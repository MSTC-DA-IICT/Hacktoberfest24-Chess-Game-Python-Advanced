...
class ChessBoard:
    ...
    def is_valid_move(self, start, end, player_color):
        ...
        print(self.board[start_x][start_y].color, player_color)
...
class Game:
    ...
    def play(self):
        while True:
            ...
            end_input = input("Enter ending location of piece (x,y): ").strip()
            ...
            try:
                ...
                start_x, start_y = map(int, [8-int(start_input[1]), ord(start_input[0])-ord('A')])
                end_x, end_y = map(int, [8-int(end_input[1]), ord(end_input[0])-ord('A')])
